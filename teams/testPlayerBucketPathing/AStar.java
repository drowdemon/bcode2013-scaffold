...
public class AStar
{
	...
	private final static int BUCKETSIZE=RobotTools.BUCKETSIZE;
	private final static int ONESQUARECOST=9;
	...
	public AStar(int mapwidth, int mapheight, AStarProblem prob, MapLocation mines[])
	{
		...
		MAPWIDTH=mapwidth/BUCKETSIZE;
		MAPHEIGHT=mapheight/BUCKETSIZE;
		map=new SearchFringe[MAPHEIGHT][MAPWIDTH];
		p=prob;
		for(int i=0; i<MAPHEIGHT; i++) //puts them in buckets
		{
	...
	private AStarPoint getpt(point center, point check) //it used to do more. //TODO inline this, when it almost certainly won't be changed anymore
	{
		...
		if(check.x<MAPWIDTH && check.y<MAPHEIGHT && check.x>=0 && check.y>=0)
		{
	...
	public ArrayList<Direction> search(point start)
	{
		...
		{	
			if(p==null)
				continue;
			toSearch.add(new AStarPoint(p.x,p.y,ONESQUARECOST+map[p.y][p.x].mine,p.fromdir,start.x,start.y,ONESQUARECOST+map[p.y][p.x].mine+this.p.heuristic(p))); //first point. It has stuff in it
		}
		//System.out.println("Initial toSearch size: " + toSearch.size());
		map[start.y/BUCKETSIZE][start.x/BUCKETSIZE]=new SearchFringe(0);
		while(toSearch.size()>0) //while it exists. if this breaks thats a bug, but still.
		{
			...
			for(AStarPoint p:getSquare(currpt)) //adds the successors
			{
				...
				int newweight=currpt.weight+map[p.y][p.x].mine+ONESQUARECOST; //weighting and heursticing
